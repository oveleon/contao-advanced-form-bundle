...
class AdvancedForm extends \Hybrid
{
    ...
    protected function compile()
    {
        ...
        if (!empty($arrFields) && \is_array($arrFields))
        {
            ...
            foreach ($arrFields as $objField)
            {
                ...
                {
                    if (isset($_SESSION[$this->formId][$objField->name]) && $_SESSION[$this->formId][$objField->name] != '')
                    {
    ...
    {
        $arrFormPages = array();
        $objFormPages = AdvancedFormPageModel::findPublishedByPid($this->id, ($_SESSION['ADVANCED_FORM_GUEST_PAGE_VISIBLE'] ?? null) ? false : true);
        ...
        if (isset($_SESSION['ADVANCED_FORM_GUEST_PAGE_VISIBLE']))
        {
        ...
        while ($objFormPages->next())
        {
            ...
            {
                if (isset($_SESSION[$this->formId][$andCondition['key']]) && $_SESSION[$this->formId][$andCondition['key']] === $andCondition['value'])
                {
                    continue;
                }
                else
            ...
            {
                if (isset($_SESSION[$this->formId][$condition['key']]) && $_SESSION[$this->formId][$condition['key']] === $condition['value'])
                {
                    $arrFormPages[] = $objFormPages->current();
                    break;
                }
    ...
    protected function getCurrentFormPage()
    {
        ...
        if (isset($_SESSION[$this->formId]['FORM_RESET']))
        {
            unset($_SESSION[$this->formId]);
            unset($_SESSION['FORM_DATA']);
        ...
        if (Input::post('FORM_SUBMIT') == preg_replace('/^auto_/i', '', $this->formId) && !isset($_SESSION[$this->formId]['SECOND_ACTION']) && isset($_SESSION[$this->formId]['FORM_PAGE_INDEX']))
        {
            ...
            if (isset($_SESSION[$this->formId]['SECOND_ACTION_INDEX']))
            {
    ...
    {
        $formPageAlias = $_SESSION[$this->formId]['FORM_PAGE'] ?? null;
        $hit = false;
    ...
    {
        $this->addPrevButton = !($this->objFormPage->hidePrevButton ?? null);
    ...
    protected function initializeSession($formId)
    {
        ...
        {
            if (isset($_SESSION[$formId][$tl]) && \is_array($_SESSION[$formId][$tl]))
            {
